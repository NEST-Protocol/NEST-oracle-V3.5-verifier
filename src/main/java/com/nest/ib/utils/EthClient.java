...
public class EthClient {
    ...
    public boolean withdrawNest(BigInteger amount, Wallet wallet) {
        ...
        try {
            ...
            if (ethSendTransaction.hasError()) {
                Response.Error error = ethSendTransaction.getError();
                log.error("WithdrawNest Transaction return failed:[msg = {}],[data = {}],[code = {}],[result = {}],[RawResponse = {}],",
                        error.getMessage(),
                        error.getData(),
                        error.getCode(),
                        ethSendTransaction.getResult(),
            ...
            log.info("WithdrawNest TransactionHash : {}", transaction);
        } catch (Exception e) {
    ...
    /**
     * Query the number of NEST defrosted
     *
     * @param miner
     * @return
     */
    public BigInteger getMinerNest(String miner) {
        ...
        try {
            ...
            log.error("Failed to get the NEST balance thawed in the contract: {}", e.getMessage());
        }
        return r;
    }
    ...
    public List<PriceSheet> unClosedSheetListOf(String miner, String tokenAddress, BigInteger maxFindNum) {
        ...
        try {
            ...
            log.error("Failed to obtain frozen assets: {}", e.getMessage());
        }
     ...
     * @return
     */
    public List<Uint32> canClosedSheetIndexs(String miner, String tokenAddress, BigInteger maxFindNum) {
        List<PriceSheet> list = unClosedSheetListOf(miner, tokenAddress, maxFindNum);
     ...
     * @return
     */
    public List<PriceSheetPub2> unVerifiedSheetList(String address) {
        List<PriceSheetPub2> priceSheetPubs = null;
        try {
            priceSheetPubs = ContractBuilder.nestMiningContract(Web3jHelper.getWeb3j()).unVerifiedSheetList(address).send();
        } catch (Exception e) {
            log.error("Failed to get unverified quotation: {}", e.getMessage());
        }
        return priceSheetPubs;
    }
